...
export function useCommonWidgetForm(
    ...
    const { data: sources = [] } = useQuery({
        ...
        queryFn: () => dataSourceRepository.findAll(),
        staleTime: 1000 * 60 * 5,
    });

    const src = sources?.find((s: DataSource) => s.id === sourceId);

    const { refetch } = useQuery({
        queryKey: ["sourceData", src?.id],
        queryFn: () => src ? dataSourceRepository.fetchData(src.id) : Promise.resolve([]),
        enabled: false,
    });
